...
class Battery:
    def __init__(self, node, level=80):
        self.level = level
        self.node = node
    ...
    def charge(self):
        while self.level < 100:
            time.sleep(0.5)
            self.level += 10
            self.node.get_logger().info(f"[배터리 충전] 충전 중... 현재 배터리 잔량: {self.level}%")
...
class CheckBattery(py_trees.behaviour.Behaviour):
    ...
    def update(self):
        ...
        self.battery.node.get_logger().info(f"[배터리 확인] 현재 배터리 잔량: {self.battery.level}%")
        if self.battery.level < 20:
            self.battery.node.get_logger().info("[배터리 확인] 배터리가 부족합니다, 충전이 필요합니다!")
            return py_trees.common.Status.FAILURE
        else:
            self.battery.node.get_logger().info("[배터리 확인] 배터리 잔량이 충분합니다.")
            return py_trees.common.Status.SUCCESS
...
class ChargeBattery(py_trees.behaviour.Behaviour):
    ...
    def update(self):
        self.battery.node.get_logger().info("[배터리 충전] 충전 시작...")
        self.battery.charge()
        self.battery.node.get_logger().info("[배터리 충전] 배터리가 완전히 충전되었습니다!")
        return py_trees.common.Status.SUCCESS

class SampleCollect(py_trees.behaviour.Behaviour):
    def __init__(self, name, node):
        super().__init__(name)
        self.node = node
    
    def update(self):
        self.node.get_logger().info("샘플 채취 중...")
        return py_trees.common.Status.SUCCESS
...
class MyBTNode(Node):
    def __init__(self):
        ...
        self.battery = Battery(node=self)
    ...
    def create_tree(self):
        ...
        battery_management = py_trees.composites.Selector(
            name="배터리 체크 및 충전 셀렉터",
            memory=False)
        battery_management.add_children([battery_check, charge_battery])

        sample_collect = SampleCollect(name="샘플 채취", node=self)
        repeat_sample_collect = py_trees.decorators.Repeat(
            name="샘플 채취 리피터",
            child=sample_collect,
            num_success=3
            )

        root.add_children(
            [obstacle_sequence, battery_management, repeat_sample_collect]
            )
